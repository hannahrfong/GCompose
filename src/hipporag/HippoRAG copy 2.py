...
class HippoRAG:
    ...
    def initialize_graph(self):
        ...
        self._entity_counts_filename = os.path.join(
            self.working_dir, f"entity_counts.json"
        )
    ...
    def run_ppr(self,
        ...
        print("BENSON BOONE 1111")
        print(damping)

        reset_prob = np.where(np.isnan(reset_prob) | (reset_prob < 0), 0, reset_prob)
        print("BENSON BOONE 2222")
        print(reset_prob)
...
# GCompose
    def save_entity_passage_counts(self, output_path: str = None):
        """
        Save per-entity passage counts and total number of passages for IDF computation.

        Saves a JSON file with the format:
        {
            "total_chunks": <int>,
            "entity_counts": {
                "entity-<hash1>": 3,
        ...
        if output_path is None:
            output_path = self._entity_counts_filename

        # Compute per-entity counts
        entity_counts = {k: len(v) for k, v in self.ent_node_to_chunk_ids.items()}

        # Total number of passages = number of passage nodes in the graph
        total_chunks = len(set().union(*self.ent_node_to_chunk_ids.values()))
        ...
        print(f"Total passages: {total_chunks}, unique entities: {len(entity_counts)}")

        with open(output_path, "w") as f:
            json.dump(data, f, indent=2)

        logger.info(f"Saved entity passage counts and total passages to {output_path}")
    ...
    def find_query_nodes(self, query_entities: List[str]) -> Dict[str, int]:
        ...
        print("testing lang")
        idx = self.entity_embedding_store.text_to_hash_id["phillip k allen"]
        print(idx)
        text = self.entity_embedding_store.hash_id_to_text[idx]
        print(text)
    ...
    def _truncate_by_token_limit(self, sorted_paths: List[Tuple[List[int], float]], max_tokens: int = 2048) -> List[List[int]]:
        """
        Truncate top paths so that the total estimated tokens do not exceed max_tokens.
        Heuristic: 1 token ≈ 4 characters.
        
        Args:
            sorted_paths: List of tuples (path: List[node_idx], score: float), sorted by score descending.
            max_tokens: Maximum allowed tokens for all paths combined.

        Returns:
            List of node index paths (List[int]) that fit within max_tokens.
        """
        truncated_paths = []
        total_chars = 0

        print("INSIDE _TRUNCATE")

        for path, score in sorted_paths:
            # Estimate number of characters in the path as sum of node name lengths
            print("TRUNCATE 1111")
            path_chars = sum(len(str(self.graph.vs[idx]["name"])) for idx in path)
            self.idx_to_entity_text[idx]
            print("TRUNCATE 2222")
            path_tokens = path_chars / 4  # heuristic: 1 token ≈ 4 chars
            print("TRUNCATE 3333")
            if total_chars / 4 + path_tokens <= max_tokens:
                print("TRUNCATE 4444")
        ...
        print(f"[DEBUG] _truncate_by_token_limit: {len(truncated_paths)} paths selected, "
            f"total estimated tokens: {total_chars / 4:.1f}/{max_tokens}")
    ...
    def extract_top_k_paths(self, 
        ...
        for i, src in enumerate(top_nodes):
            ...
            for dst in top_nodes[i+1:]:
                ...
                found_paths = subgraph.get_all_simple_paths(src_idx, to=dst_idx, cutoff=n_hop, mode="all")
                ...
                for path in found_paths:
                    ...
                    for j, idx in enumerate(path):
                        node_name = subgraph.vs[idx]["name"]
